...
class BoardGamesPlugin(Star):
    ...
    async def clock_help(self, event: AstrMessageEvent):
        ...
        yield event.plain_result(
            "计时规则\n"
            "【简介】双方完成选边后开钟，耗尽即负；剩余时间显示在棋盘顶部。\n"
            "【详细】支持三种格式：\n"
            "分钟+每步加秒、分钟包干、围棋基本用时|次数x读秒秒数。\n"
    ...
    @filter.command(
        "等级分说明", alias={"计分说明", "Elo说明", "elo说明", "等级分帮助"}
    )
    async def rating_help(self, event: AstrMessageEvent):
        """说明排行榜使用的 Elo 等级分算法。"""
        yield event.plain_result(
            "等级分说明\n"
            "【简介】每个棋种分别计算 Elo，初始 1000 分。击败强手涨得多，"
            "击败弱手涨得少；和棋也会按双方实力调整。\n"
            "【详细】插件使用 K=32：预期胜率 E=1/(1+10^((对手分-自己分)/400))，"
            "本局得分 S 为胜1、和0.5、负0，变化量取整为 32×(S-E)。"
            "理论上双方变化量相反；实际应用100分下限。\n"
            "同为1000分时，胜者约+16、负者-16，和棋不变；"
            "若相差200分，强方正常获胜约+8，弱方爆冷获胜约+24。\n"
            "认输、棋钟超时、协议和棋和规则终局都会计入；流局不计。\n"
            "/对局记录 的胜率是胜局数÷总局数，和棋计入总局但不算胜局。"
            "排行榜等级分与 /AI分析 的局面胜率没有直接关系。"
        )
    ...
    async def go_scoring_help(self, event: AstrMessageEvent):
        ...
        yield event.plain_result(
            "围棋计分\n"
            "【简介】双方连续 pass 后，按中国规则的面积法自动数子；"
            "这和局中的 AI 估分不是一回事。\n"
            "【详细】黑白分别计算棋盘上的活子，加上只被己方包围的空点；"
    ...
    async def help(self, event: AstrMessageEvent):
        ...
        yield event.plain_result(
            "多棋盘对弈｜简介\n"
            "支持国际象棋、围棋、中国象棋、五子棋、井字棋和黑白棋。\n"
            "三步开局：\n"
            "1. /开局 围棋（棋种可替换）\n"
            "2. 对手 /加入棋局\n"
            "3. 任一人 /选先 或 /选后，对手自动获得另一方\n"
            "轮到自己后可直接发送走法，例如 Nc3、D4、炮二平五或 5。\n"
            "完整指令：/棋类详细帮助\n"
            "走法格式：/下棋帮助\n"
            "棋种规则：/国际象棋帮助 /围棋帮助 /中国象棋帮助 "
            "/五子棋帮助 /井字棋帮助 /黑白棋帮助"
        )

    @filter.command("棋类详细帮助", alias={"棋类完整帮助", "棋盘游戏详细帮助"})
    async def detailed_help(self, event: AstrMessageEvent):
        """显示完整指令索引。"""
        yield event.plain_result(
            "多棋盘对弈｜详细帮助\n"
            "【开局】/开局 [棋种] [规则/路数] [计时=…/不计时]\n"
            "示例：/开局 国际象棋 计时=15+10\n"
            "      /开局 围棋 19路 计时=60|3x30\n"
            "      /开局 五子棋 连珠 Swap2\n"
            "【入场与选边】/加入棋局；/选先 /选后 /选黑 /选白 /选红\n"
            "【对局】/棋盘 /棋谱 /棋钟 /悔棋 /和棋 /流局 /认输\n"
            "同意或拒绝：/同意悔棋 /拒绝悔棋 /同意和棋 /拒绝和棋 "
            "/同意流局 /拒绝流局\n"
            "【分析】/AI分析；围棋可用 /势力范围；对手用 /同意AI 或 /拒绝AI\n"
            "【记录】/战绩；/对局记录 [@玩家] [棋种]；/排行榜 [棋种]\n"
            "【说明】/下棋帮助 /计时规则 /等级分说明 /围棋计分\n"
            "【管理】/强制流局；空房未有人加入时，群内任何人可直接 /流局\n"
            "【Swap2】按棋盘提示使用 /选白 /选黑 /交换 /加两子\n"
            "所有非走子指令必须保留 /；完整规则请使用对应棋种帮助。"
        )
    ...
    async def move_help(self, event: AstrMessageEvent):
        yield event.plain_result(
            "下棋帮助\n"
            "【简介】轮到自己时可发送 /下棋 Nc3、/Nc3 或直接 Nc3。\n"
            "【详细】\n"
    ...
    async def chess_help(self, event: AstrMessageEvent):
        yield event.plain_result(
            "国际象棋帮助\n"
            "【简介】8×8 完整国际象棋，默认用时15+10，白方先行。\n"
            "【详细】开局：/开局 国际象棋 → /加入棋局 → 任一人 /选白 或 /选黑。\n"
    ...
    async def go_help(self, event: AstrMessageEvent):
        yield event.plain_result(
            "围棋帮助\n"
            "【简介】支持 9、13、19 路，默认标准 19 路、60|3x30，黑方先行。\n"
            "【详细】开局：/开局 围棋 [9路/13路/19路]。\n"
    ...
    async def xiangqi_help(self, event: AstrMessageEvent):
        yield event.plain_result(
            "中国象棋帮助\n"
            "【简介】完整中国象棋规则，默认用时10+5，红方先行。\n"
            "【详细】开局：/开局 中国象棋，任一人 /选红 或 /选黑。\n"
    ...
    async def gomoku_help(self, event: AstrMessageEvent):
        yield event.plain_result(
            "五子棋帮助\n"
            "【简介】可选自由、标准或连珠规则，也可启用Swap2平衡开局。\n"
            "【详细】开局：/开局 五子棋 [自由/标准/连珠] [Swap2] [13/15/19路]。\n"
    ...
    async def tictactoe_help(self, event: AstrMessageEvent):
        yield event.plain_result(
            "井字棋帮助\n"
            "【简介】固定视角的3×3井字棋，X先行，默认用时2+2。\n"
            "【详细】开局：/开局 井字棋，任一人 /选先 或 /选后。\n"
    ...
    async def reversi_help(self, event: AstrMessageEvent):
        yield event.plain_result(
            "黑白棋帮助\n"
            "【简介】8×8黑白棋，黑方先行，默认每方10分钟包干。\n"
            "【详细】开局：/开局 黑白棋，任一人 /选黑 或 /选白。\n"
